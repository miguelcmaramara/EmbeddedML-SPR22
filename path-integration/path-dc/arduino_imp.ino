...
void loop() {
  ...
  if(digitalRead(SWITCH_PIN) == HIGH && IMU.accelerationAvailable() && IMU.gyroscopeAvailable()){
    ...
    while(digitalRead(SWITCH_PIN) == HIGH && numData < INPUT_LENGTH){
      ...
      Serial.print(", ");
      Serial.print(zIn[numData]);
      Serial.print(", ");
      Serial.print(gx[numData]);
      Serial.print(", ");
      Serial.print(gy[numData]);
      Serial.print(", ");
      Serial.println(gz[numData]);
    ...
    //old tensor loaded notification
    //Serial.println("Tensor Loaded");

    // Perform inference

    TfLiteStatus invoke_status = interpreter->Invoke();
        //old inference invocation notification
        //Serial.println("Invoked inference");
  if (invoke_status != kTfLiteOk) {
    TF_LITE_REPORT_ERROR(error_reporter, "Invoke failed on index: %d\n");
    return;
  } else{
    //old error test passed notification
    //Serial.println("Error test passed");

    //Output results
    float *gesture_pred = interpreter->output(0)->data.f;
    //old output assignment notification
    //Serial.println("Output assigned");
    //prints predictions in the order of 1-5 in own rows due to Serial.print() constraints
    Serial.println(gesture_pred[0]);
    Serial.println(gesture_pred[1]);
    Serial.println(gesture_pred[2]);
    Serial.println(gesture_pred[3]);
    Serial.println(gesture_pred[4]);

    //writeBLE(gesture_pred);

    //old gesture procedural notifications
    /*Serial.println("1");
    if(gesture_pred[0] > .5){
